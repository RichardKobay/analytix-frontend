<template>
  <div v-if="loading" class="w-screen h-screen flex flex-col items-center justify-center bg-gray-200">
    <div class="text-center">
      ...
      <p class="text-xl font-bold mt-4">{{ loadingText }}</p>
    </div>
  </div>
...
const handleUserSent = async (username) => {
  usernameReceived.value = username
  console.log('Received username:', usernameReceived.value)

  loading.value = true
  animateLoadingText()

  try {
    const response = await fetch(`http://localhost:8000/api/user-tweets/${username}`)
    if (!response.ok) {
...
const animateLoadingText = () => {
  ...
  loadingInterval = setInterval(() => {
    loadingText.value = 'Loading' + dots[index]
